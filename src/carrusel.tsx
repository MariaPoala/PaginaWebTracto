...
export default function HeroCarousel({
  ...
  return (
    <section
      ...
      className={
        [
          ...
          // Fondo + borde sutil con la paleta rojo/naranja
          "bg-neutral-950/70 ring-1 ring-white/10 shadow-2xl",
          "supports-[backdrop-filter]:backdrop-blur-sm",
          // Glow cálido
          "shadow-[0_10px_40px_rgba(234,88,12,0.25)]",
          className || "",
        ].join(" ")
      }
    ...
    >
      {/* Barra de progreso con gradiente rojo→naranja */}
      <div className="absolute left-0 top-0 z-30 h-1 w-full bg-white/10">
        <div
          className="h-full bg-gradient-to-r from-red-500 via-orange-500 to-amber-400 transition-[width]"
          style={{ width: `${Math.min(progress * 100, 100)}%` }}
        />
      </div>

      {/* Mallas decorativas cálidas */}
      <div
        aria-hidden
        className="pointer-events-none absolute -left-1/4 -top-1/3 z-0 h-[60vh] w-[60vw] rounded-full bg-gradient-to-br from-red-600/25 via-orange-500/20 to-amber-400/20 blur-3xl"
      />
      <div
        aria-hidden
        className="pointer-events-none absolute -right-1/3 -bottom-1/3 z-0 h-[60vh] w-[60vw] rounded-full bg-gradient-to-tr from-orange-500/25 via-amber-400/20 to-rose-400/20 blur-3xl"
      />
          ...
          <article key={s.id} className="relative h-full w-full shrink-0">
            ...
            <div className="absolute inset-0 overflow-hidden">
              <img
                ...
                decoding="async"
              />
              {/* Vignette/overlay cálido */}
              <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/35 to-black/20" />
              <div className="absolute inset-0 bg-[radial-gradient(60%_60%_at_70%_90%,rgba(234,88,12,0.18),transparent)]" />
            </div>
              ...
              <div className="container mx-auto grid h-full grid-cols-1 px-6 md:px-10">
                ...
                >
                  ...
                  {s.cta && (
                    <a
                      href={s.cta.href}
                      className="mt-6 inline-flex items-center gap-2 rounded-full bg-gradient-to-tr from-red-600 via-orange-500 to-amber-400 px-5 py-2.5 text-sm font-medium text-white shadow-lg ring-1 ring-white/10 transition focus:outline-none focus:ring-2 focus:ring-white/40 hover:brightness-110 active:scale-[.98] md:text-base"
                    >
                      {s.cta.label}
                      <svg
      ...
      <button
        aria-label="Anterior"
        onClick={prev}
        className="group absolute left-3 top-1/2 z-30 -translate-y-1/2 rounded-2xl bg-white/10 p-3 text-white ring-1 ring-white/30 backdrop-blur transition hover:bg-orange-400/25 focus:outline-none focus:ring-2 focus:ring-orange-300/60"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
      ...
      <button
        ...
        onClick={next}
        className="group absolute right-3 top-1/2 z-30 -translate-y-1/2 rounded-2xl bg-white/10 p-3 text-white ring-1 ring-white/30 backdrop-blur transition hover:bg-red-500/25 focus:outline-none focus:ring-2 focus:ring-red-400/60"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
        ...
        {slides.map((s, i) => (
          <button
            ...
            className={[
              ...
              i === index
                ? "scale-110 bg-gradient-to-r from-red-500 via-orange-500 to-amber-400"
