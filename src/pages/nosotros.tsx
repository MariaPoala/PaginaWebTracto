...
function AccentTitle({ children, subtitle }: { children: ReactNode; subtitle?: string }) {
  return (
    <div className="mb-6">
      <h2 className="text-2xl font-extrabold tracking-tight text-emerald-900 sm:text-3xl">{children}</h2>
...
function Pillar({ icon: Icon, title, text }: { icon: LucideIcon; title: string; text: string }) {
  return (
    <div className="group relative rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition duration-300 hover:-translate-y-0.5 hover:shadow-xl">
      <div className="mb-3 inline-flex h-12 w-12 items-center justify-center rounded-xl bg-emerald-100 text-emerald-600 transition group-hover:scale-105">
...
function Timeline() {
  return (
    <div className="relative">
      <div className="absolute left-4 top-0 h-full w-px bg-gradient-to-b from-emerald-300 via-green-300 to-emerald-300" />
      <ul className="space-y-6">
        {TIMELINE.map((i) => (
          <li key={i.year} className="relative pl-12">
            <div className="absolute left-1 top-2 h-6 w-6 rounded-full bg-gradient-to-tr from-emerald-700 to-green-600 ring-4 ring-emerald-100" />
            <p className="text-sm font-semibold text-emerald-900">{i.year}</p>
    ...
    <main className="bg-white">
      ...
      <Section>
        <div className="grid grid-cols-1 items-center gap-10 lg:grid-cols-2">
          <div>
            <Badge className="bg-emerald-50 text-emerald-700 ring-emerald-200 inline-flex items-center gap-2">
              <Building2 className="h-4 w-4" /> Nosotros
            </Badge>
            <motion.h1 {...fadeUp} className="mt-4 text-3xl font-black tracking-tight text-emerald-900 sm:text-4xl">
          ...
          <motion.div {...fadeUp} className="grid grid-cols-2 grid-rows-2 gap-3">
            <div className="col-span-1 row-span-2 overflow-hidden rounded-2xl border border-emerald-100 bg-slate-100">
            ...
            <div className="overflow-hidden rounded-2xl border border-emerald-100 bg-slate-100">
      ...
      <Section>
        <div className="relative overflow-hidden rounded-2xl border border-emerald-100 bg-gradient-to-r from-emerald-50 to-teal-50 p-6">
          <div className="grid grid-cols-1 items-center gap-6 md:grid-cols-2">
            <div>
              <h3 className="text-2xl font-extrabold text-emerald-900">¿Listo para trabajar con nosotros?</h3>
            ...
            <div className="flex flex-wrap items-center justify-start gap-3 md:justify-end">
              <GradientButton href="#contacto">Escríbenos ya <ArrowRight className="h-4 w-4" /></GradientButton>
              <a
                href="https://wa.me/51981830008"
                className="inline-flex items-center gap-2 rounded-xl border border-emerald-300 bg-white px-4 py-2 text-slate-800 transition hover:border-emerald-400 hover:bg-emerald-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400"
              >
