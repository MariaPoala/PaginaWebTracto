...
function Badge({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
    <span className={`inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 ring-1 ring-emerald-200 ${className}`}>
...
function Stat({ icon: Icon, label, value }: { icon: any; label: string; value: string }) {
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="inline-flex h-10 w-10 flex-none items-center justify-center rounded-xl bg-emerald-100 text-emerald-600">
        <Icon className="h-5 w-5" />
      </div>
      <div>
        <p className="text-xs uppercase tracking-wide text-slate-500">{label}</p>
        <p className="text-lg font-extrabold text-emerald-900">{value}</p>
      </div>
    </div>
  );
}
...
function PlanCard({ name, price, features, highlight = false }: { name: string; price: string; features: string[]; highlight?: boolean }) {
  return (
    ...
    >
      {highlight && (
        <div className="absolute -top-3 right-4 rounded-full bg-gradient-to-r from-emerald-700 to-green-600 px-3 py-1 text-[11px] font-bold uppercase tracking-wide text-white shadow">
          Más vendido
        </div>
      )}
      <h3 className="text-lg font-extrabold text-emerald-900">{name}</h3>
      <p className="mt-1 text-2xl font-black text-slate-900">{price}</p>
      <ul className="mt-4 space-y-2 text-sm text-slate-700">
        {features.map((f, i) => (
          <li key={i} className="flex items-start gap-2">
            <CheckCircle2 className="mt-0.5 h-4 w-4 text-emerald-600" /> {f}
          </li>
        ))}
      </ul>
      <a
        href="/solicitar-servicio"
        className="mt-5 inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-emerald-700 to-green-600 px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:from-emerald-800 hover:to-green-700"
      >
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4">
      ...
      >
        ...
        <span className={`ml-4 text-emerald-600 transition ${open ? "rotate-90" : ""}`}>›</span>
      </button>
...
export default function Servicios() {
  return (
    <main className="bg-white">
        ...
        <div className="grid grid-cols-1 items-center gap-8 lg:grid-cols-2">
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <a
                href="#docs"
                className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-emerald-700 to-green-600 px-4 py-2 text-white shadow-sm transition hover:from-emerald-800 hover:to-green-700"
              >
              ...
              <a
                ...
                className="inline-flex items-center gap-2 rounded-xl border border-emerald-300 bg-white px-4 py-2 text-emerald-700 transition hover:bg-emerald-50"
              >
      ...
      <Section>
        <div className="mb-6">
          <h2 className="text-2xl font-black text-emerald-900 sm:text-3xl">Servicios</h2>
      ...
      <Section>
        <div className="mb-6">
          <h2 className="text-2xl font-black text-emerald-900 sm:text-3xl">Cómo trabajamos</h2>
        ...
        <ol className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
          {["Solicitud", "Diagnóstico", "Aprobación", "Reparación", "Entrega"].map((step, i) => (
            <li key={step} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-100 font-bold text-emerald-700">{i + 1}</div>
      ...
      <Section>
        <div className="grid grid-cols-1 items-center gap-6 lg:grid-cols-2">
          <div className="order-2 lg:order-none">
            <h2 className="text-2xl font-black text-emerald-900 sm:text-3xl">Cobertura nacional y SLA</h2>
            ...
            <ul className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
              <li className="rounded-2xl border border-slate-200 bg-white p-4 text-slate-800 shadow-sm"><Clock className="mr-2 inline h-5 w-5 text-emerald-600"/> Respuesta estándar: ≤ 48h</li>
              <li className="rounded-2xl border border-slate-200 bg-white p-4 text-slate-800 shadow-sm"><ShieldCheck className="mr-2 inline h-5 w-5 text-emerald-600"/> Garantía hasta 12 meses</li>
              <li className="rounded-2xl border border-slate-200 bg-white p-4 text-slate-800 shadow-sm"><Truck className="mr-2 inline h-5 w-5 text-emerald-600"/> Taller móvil en regiones</li>
              <li className="rounded-2xl border border-slate-200 bg-white p-4 text-slate-800 shadow-sm"><Wrench className="mr-2 inline h-5 w-5 text-emerald-600"/> Repuestos disponibles</li>
            </ul>
          </div>
      ...
      <Section>
        <div className="rounded-2xl border border-emerald-200 bg-gradient-to-r from-emerald-50 to-teal-50 p-6 text-center">
          <h3 className="text-xl font-black text-emerald-900">¿Necesitas servicio técnico ahora?</h3>
          <p className="mt-1 text-slate-700">Cuéntanos tu caso y un asesor te contactará de inmediato.</p>
          <div className="mt-4 flex flex-wrap items-center justify-center gap-3">
            <a href="#docs" className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-emerald-700 to-green-600 px-4 py-2 text-white shadow-sm transition hover:from-emerald-800 hover:to-green-700">
              Solicitar servicio <ArrowRight className="h-4 w-4" />
            </a>
            <a href="tel:+51981830008" className="inline-flex items-center gap-2 rounded-xl border border-emerald-300 bg-white px-4 py-2 text-emerald-700 transition hover:bg-emerald-50">
